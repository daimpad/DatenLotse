...
| `tests/hvd.spec.js` | Einstufung bleibt manuell, Vorschlagsheuristik, alle Pflichten der DVO (EU) 2023/138, Export nur gemeinsam, CSV-/Katalog-Round-Trip |
...
| Konstante | Inhalt |
|---|---|
| `HVD_CATEGORIES` / `HVD_META` / `HVD_OPTIONS` | die sechs Kategorien (Georaum, Erdbeobachtung und Umwelt, Meteorologie, Statistik, Unternehmen, Mobilität) mit Beispielen |
| `HVD_ELI` | `http://data.europa.eu/eli/reg_impl/2023/138/oj` |
| `HVD_SCHEME` | Vokabular „High-value dataset categories" der EU-Publikationsstelle |
...
- **Die Einstufung bleibt beim Menschen.** `guessHvd(d)` *schlägt vor*, es setzt nichts – ob die Verordnung greift, ist eine Rechtsfrage und keine, die eine Stichwortliste entscheiden darf. Die Vorschläge stehen als eigener, akzentfarbener Block (`#quality-hvd`, `renderHvdHinweise()`) über den Befunden und **färben keine Ampel**: eine Vermutung ist kein Befund. Die Heuristik ist bewusst eher großzügig (ein übersehener HVD wiegt schwerer als ein Vorschlag zu viel), lässt aber **personenbezogene und nicht öffentliche Bestände aus** – dort wäre der Vorschlag nur Lärm.
...
- **`dcatap:applicableLegislation` und `dcatap:hvdCategory` werden nur GEMEINSAM geschrieben** (JSON-LD und Turtle, Präfix `dcatap: http://data.europa.eu/r5r/`). Ein Datensatz, der sich als HVD ausweist, ohne eine Kategorie zu nennen, fällt beim Harvesting durch die SHACL-Validierung – dann lieber gar keine HVD-Angabe, und die Qualitätsprüfung sagt, was fehlt.

> ⚠️ **`HVD_CATEGORIES[].uri` ist absichtlich leer.** Die sechs amtlichen Kategorie-URIs ließen sich aus der Entwicklungsumgebung heraus nicht gegen das Vokabular der EU-Publikationsstelle prüfen (`data.europa.eu` gesperrt), und eine geratene URI wäre in einem Rechtskontext das Schlechteste, was das Werkzeug ausgeben kann – dieselbe Linie wie beim `@base` im Turtle-Export. Bis dahin trägt der Datensatz die URI im Feld `hvdCategoryURI` selbst; die Karte verlinkt das Vokabular und erklärt warum. **Sobald die URIs verifiziert sind, genügt es, `uri` je Kategorie zu füllen** – `hvdCategoryUri()` und `hvdFromURI()` greifen dann automatisch, inklusive Rückabbildung beim Katalog-Import. Ein Test hält fest, dass keine Kategorie eine URI außerhalb von `data.europa.eu/bna/` führt.
...
| Hochwertige Datensätze (HVD) | `hvdIssues()`, `guessHvd()`, `hvdVorschlaege()`, `hvdCategoryUri()`, `hvdFromURI()`, `renderHvdHinweise()`, `HVD_CATEGORIES`, `HVD_LICENSES`, `HVD_ELI` | `[data-field="hvd"]`, `[data-field="hvdCategoryURI"]`, `.inv-hvd`, `#quality-hvd`, `.qual-hvd` |

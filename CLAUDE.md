...
- **Aktuelle Version:** `v65` (Script-Tag: `<script src="js/app.js?v=65">`)
...
| `datenlotse_kompass_profil` | Angabe zur Rechtspflicht (`kompassProfil`, `{ rechtspflicht: ''\|'ja'\|'nein'\|'unklar' }`) als JSON |
...
> ⚠️ **`official-work` ist der einzige Eintrag, den nicht jede Organisation wählen darf.** Die Lizenzfreiheit amtlicher Werke nach § 5 UrhG hängt an der **amtlichen Herkunft**, nicht an einer Entscheidung des Herausgebers – ein Verein, der sie wählt, veröffentlicht eine Lizenzangabe ohne Grundlage. `open: true` ist trotzdem richtig, die Prüfung greift also nicht über `licenseIsOpen()`. Seit v56 meldet `validateDataset()` das als **Fehler**, aber nur bei `kompassProfil.rechtspflicht === 'nein'` – ohne Angabe bleibt es wie bisher (dieselbe Zurückhaltung wie überall bei dieser Angabe). Der Eintrag bleibt im Dropdown: fremde Kataloge können ihn mitbringen, und Ausblenden würde ihn beim Rückimport still verschlucken.
...
### Lizenz-Kompatibilität (`licenseCompat`)

Das Register listet 20 Lizenzen, sagte aber nichts über ihr **Zusammenspiel**. Wer eigene Daten mit einer fremden Quelle verbindet, erfuhr das Problem erst hinterher.

`LICENSE_TRAITS` trägt je Lizenz `{ by, sa, nc, nd, frei?, amtlich? }`. **`sa` benennt die Copyleft-Familie, nicht nur ihr Vorhandensein** – CC BY-SA und ODbL verlangen beide das Ergebnis unter sich selbst und sind deshalb gegenseitig unvereinbar; zwei Mal `true` würde diesen Fall verfehlen. Die beiden Sammelposten `other-open` und `other-closed` stehen bewusst **nicht** in der Tabelle: sie benennen keinen Lizenztext, das Ergebnis ist `unklar`.

`licenseCompat(a, b)` liefert `{ status, pflichten, gruende }` mit `status` ∈ `ok` / `achtung` / `unvereinbar` / `unklar`. Die Regeln, geordnet:

| Regel | Ergebnis | Warum |
|---|---|---|
| eine Seite trägt **ND** | `unvereinbar` | eine Zusammenführung *ist* eine Bearbeitung |
| zwei **verschiedene** Copyleft-Familien | `unvereinbar` | jede verlangt das Ergebnis unter sich selbst |
| Copyleft **ohne** NC trifft auf **NC** | `unvereinbar` | das Ergebnis müsste kommerzielle Nutzung zugleich erlauben und verbieten |
| irgendeine Seite **NC** | `achtung` | zusammenführbar, aber nach der Open Definition nicht mehr offen |
| sonst | `ok` | `pflichten` nennt Namensnennung / Copyleft |

> ⚠️ **Ein Hinweis, kein Urteil.** Das Werkzeug benennt die Pflichten aus den Lizenztexten und wo sie einander widersprechen – ob eine Zusammenführung im Einzelfall zulässig ist, entscheidet es nicht. Der Ergebniskasten sagt das ausdrücklich („keine Rechtsberatung").

**Zwei Einstiege:** die Prüfung im Lizenz-Wegweiser (`renderLicenseCompat()`, eigener State `licCompat` – die Empfehlung oben bleibt davon unberührt) und `validateDataset()`, das **verschiedene Lizenzen an den Verteilungen desselben Datensatzes** als Warnung meldet (DCAT-AP.de erlaubt das, für Nachnutzende ist es aber eine Falle: wer die CSV nimmt, unterliegt anderen Bedingungen als wer die JSON nimmt). Sind die beiden zusätzlich unvereinbar, sagt die Meldung das mit.
...
> ⚠️ **Ein Ergebnis darf nie älter sein als die Eingabe, aus der es stammt.** Seit v61 verwerfen alle drei Tabs der Textbereinigung ihre Ausgabe, sobald sich die Grundlage ändert: `pseudoCsvInvalidate()` bei jeder Änderung der Spaltenkonfiguration, `pseudoTextInvalidate()` bei jeder Änderung des Eingabetexts, `riskInvalidate()` bei jeder Änderung der Merkmalsauswahl (v57). Der Grund ist nicht Kosmetik – an jeder Ausgabe hängt ein Download, der den Stand des **letzten Laufs** festhält. Vorher konnte auf dem Schirm „ganze Spalte ersetzen" stehen, während die heruntergeladene Datei den Wert weiter im Klartext trug.
>
> Dieselbe Regel gilt fürs Rendern: das Umschalten eines einzelnen Feldes aktualisiert **nur** das abhängige Feld, nie die ganze Box – sonst verliert das gerade bediente Element den Tastaturfokus.
...
### Re-Identifikationsrisiko: k-Anonymität & l-Diversität

Dritter Tab der Textbereinigung (`#pseudo-tab-risiko` → `#pseudo-risiko-panel`). Die Spaltenbereinigung sagt, **was** ersetzt wurde – nie, ob das Ergebnis noch auf einzelne Personen zurückführt. Vier Spalten pseudonymisieren und trotzdem eine Zeile behalten, die als einzige „1948 · 04315 · weiblich" trägt, ist keine Anonymisierung.

- **Nur messen, nie verändern.** Generalisieren und Unterdrücken (wie ARX es kann) wäre eine Entscheidung über fremde Daten. Das Werkzeug nennt das Risiko, die Entscheidung bleibt beim Menschen. Aus demselben Grund **wählt der Mensch die Quasi-Identifikatoren** – eine Heuristik, die rät, welche Spalte identifizierend ist, täte nur so, als wüsste sie es.
- **Die harte Regel gilt hier genauso:** reine Arithmetik, Gruppen bilden und zählen. Kein ML, kein NER, keine Deutung von Inhalten.
- `equivalenceClasses(rows, qi)` gruppiert über `riskKey()`; `kAnonymitaet()` ist die Größe der kleinsten Klasse, `lDiversitaet()` die kleinste Zahl **verschiedener** Werte des sensiblen Merkmals je Klasse (distinct l-diversity).
- **`RISK_SEP` ist `\u0000` (NUL), kein druckbares Zeichen.** Mit einem druckbaren Trenner fielen `["a b", "c"]` und `["a", "b c"]` auf denselben Schlüssel – zwei Gruppen würden zu einer verschmelzen und k **größer** ausfallen, das Ergebnis sähe also **sicherer** aus als es ist. Ein Test hält das fest.
- **Normalisierung bewusst minimal:** nur Leerraum am Rand wird getrimmt, Groß-/Kleinschreibung **nicht** zusammengefasst. Zusammenfassen vergrößert Gruppen und wirkt in die gefährliche Richtung; im Zweifel lieber eine Warnung zu viel. Leere Zellen sind ein eigener Wert.
- **`K_SCHWELLE = 5`** ist eine gängige Schwelle der statistischen Geheimhaltung und im UI ausdrücklich als Konvention benannt, **nicht** als Rechtsvorschrift. Ampel: `k = 1` rot, `l = 1` gelb (auch bei großem k – wer die Gruppe kennt, kennt dann den Wert), `k < 5` gelb, sonst grün.
- **Das sensible Merkmal darf kein Quasi-Identifikator sein.** Sonst wäre es innerhalb der Klasse konstant und `l` immer 1 – eine Aussage, die nur die eigene Auswahl spiegelt. Das Dropdown blendet gewählte QI aus, `riskReport()` fängt den Fall zusätzlich ab (`sensKonflikt`).
- **Übergabe aus der Spaltenbereinigung** (`riskFromCleaned()`, Button „Risiko prüfen" im Ergebniskopf): erst bereinigen, dann prüfen, ob es trägt – genau der Weg, um den es geht.
- `buildRiskCSV()` exportiert die Gruppenübersicht (gewählte Merkmale + Gruppengröße), **nie die Originalzeilen**.
- **Jede Änderung an der Auswahl verwirft ein vorhandenes Ergebnis** (`riskInvalidate()`). Ein stehengelassener Bericht wäre die gefährlichste Variante: nach dem Wechsel des sensiblen Merkmals stünde eine Freigabe auf dem Schirm, die für eine andere Spalte berechnet wurde. Der Tab-Wechsel allein verwirft nichts.
- **Das Umschalten eines Merkmals rendert nur das Auswahlfeld neu** (`refreshRiskSens()`), nicht die ganze Box – sonst ginge der Tastaturfokus verloren. Dieselbe Regel wie bei der Massenbearbeitung im Inventar.
- **Die Zusammenfassung trägt die Ampel im linken Balken und im Icon, nicht als Hintergrund.** Weiße Schrift auf `--ampel-gelb` erreicht 3,0 : 1 und auf `--ampel-gruen` 3,4 : 1 – bei 14 px verlangt WCAG AA 4,5 : 1. ⚠️ Die **bestehenden** Badges (`.gov-score-badge--gelb`, Clearing- und Qualitäts-Zusammenfassung) haben dieselbe Schwäche und sind noch nicht nachgezogen.
...
- **Reifegrad:** `GOV_QUESTIONS` (8 gewichtete Fragen, Σ Gewichte = 100). `reifegrad()` summiert `weight × factor` (Ja = 1, Teilweise = 0,5, Nein = 0) und normiert über die Summe der **bewertbaren** Gewichte → 0–100. Ampel: ≥ 80 grün/„Reif", ≥ 50 gelb/„Im Aufbau", sonst rot/„Lückenhaft". Live-Balken je Kategorie.
- **„Nicht relevant" (`na`, seit v56):** vierte Antwortoption in `GOV_OPTS`, wortgleich zu `KOMPASS_STATUS`. Sie nimmt die Frage **aus der Wertung**, statt sie mit 0 zu bewerten. `reifegrad()` liefert dafür zusätzlich `moeglich` (Summe der bewertbaren Gewichte) und markiert abgewählte Fragen im `breakdown` mit `factor: null`; `roleGap()` wertet `na` **nicht** als Lücke. Ohne ein einziges `na` ist `moeglich === 100` und das Ergebnis **rechnerisch identisch zu vorher** – es braucht keine Migration.

  > ⚠️ **Warum das kein Komfort-Feature ist:** die DSB-Frage wiegt 12 Punkte, nach § 38 BDSG muss aber erst ab 20 Personen mit ständiger automatisierter Verarbeitung jemand bestellt werden. Ein kleiner Verein konnte diese Punkte nie erreichen – der Fragebogen wies eine Lücke aus, die keine war. Derselbe Fehler wie bei `strategie.recht` vor v55. Betrifft ebenso Behörden, die Aufgaben ausgelagert haben.
  >
  > Sind **alle acht** Fragen abgewählt, gibt es keinen Nenner: `moeglich === 0`, `score === 0`, und Badge wie PDF-Bericht schreiben „Keine bewertbare Frage" statt „Lückenhaft" – die Ampel würde sonst eine Aussage über etwas treffen, das gar nicht bewertet wurde. Das Dashboard prüft deshalb `g.moeglich`, nicht nur die Zahl der Antworten.
...
> ⚠️ **Herkunft der Bundes-/EU-Einträge (seit v62).** `LEGAL_BASIS` führt seit v62 auch **Data Act** (VO (EU) 2023/2854) und **Data Governance Act** (VO (EU) 2022/868). Beide Angaben stammen aus einem **Recherchebericht des Nutzers**, nicht aus einer eigenen Prüfung an der amtlichen Quelle – die Netzwerkrichtlinie der Entwicklungsumgebung sperrt `eur-lex.europa.eu` und `gesetze-im-internet.de`. Verlinkt ist deshalb nur, was sich aus dem dokumentierten CELEX-Schema sicher bilden lässt (die EU-Verordnungen); die deutschen Durchführungsgesetze **DADG** und **DGG** sind im Fließtext benannt, aber **bewusst nicht verlinkt** – eine geratene Fundstelle stünde sonst als „amtliche Quelle" im Werkzeug. Ein Test hält genau das fest.
...
Eigene View `#kompass-view` und **Haupt-CTA** (Hero-Button + Topbar-„Loslegen"). Eine ausführliche Open-Data-Reifegrad-Checkliste nach anerkannten Modellen: **World-Bank ODRA**, **EU Open Data Maturity**, **5-Sterne-Open-Data** (Berners-Lee), **DCAT-AP.de** und **DSGVO/FAIR** sowie **Data Orchard Data Maturity** (Quellen im UI unter „Methodik & Quellen").

- **Struktur:** `KOMPASS_DIMENSIONS` (8 Dimensionen × je 3–5 Items, 33 Prüfpunkte). Status je Item: `offen` / `teilweise` / `erfuellt` / `na`.
...
#### Rechtspflicht der Organisation (`kompassProfil`)

Eine **einzige, optionale** Angabe unter dem Score (`renderKompassProfil()`, `#kompass-profil`): *„Unterliegt Ihre Organisation einem Informationsfreiheits- oder Open-Data-Gesetz?"* – `ja` / `nein` / `unklar` / keine Angabe, persistiert unter `datenlotse_kompass_profil`, additiv in `buildProjectJSON()` (kein Schema-Bump).
...
| Stelle | Wirkung |
|---|---|
| `kompassDerived('strategie.recht')` | liefert `'na'` statt `'offen'` – ohne Pflicht gibt es keine Rechtsgrundlage zu klären, und `na` fällt aus der Wertung. Es ist der **einzige** der 33 Prüfpunkte, der strikt an einer Rechtspflicht hängt (nachgezählt, nicht geschätzt) |
| `hvdVorschlaege()` | gibt `[]` zurück – die DVO (EU) 2023/138 bindet öffentliche Stellen; ein Vorschlag „das ist ein hochwertiger Datensatz" wäre bei einem Verein nicht nur überflüssig, sondern irreführend |
| `validateDataset()` (seit v56) | meldet die Lizenz `official-work` (§ 5 UrhG) als **Fehler** – siehe Lizenz-Register |

Alles Übrige – Portal, Harvesting, DCAT-AP.de, Lizenzen, Clearing, Datenschutz – gilt **unverändert**. Die Angabe blendet nichts aus; wer sie leer lässt, bekommt exakt das Verhalten vor v55.

**Optional aus Prinzip:** DatenLotse startet ohne Einrichtung. Eine Pflichtfrage vor dem ersten Blick erzwänge eine Entscheidung vor dem Verstehen – deshalb steht sie *unter* dem Score, nicht davor.
...
**Ampel: Fläche vs. Text.** Seit v60 gibt es je Ampelstufe **zwei** Token. `--ampel-rot/gelb/gruen` bleiben für Flächen, Balken, Punkte und Rahmen; `--ampel-*-text` gilt für **Textfarben und bedeutungstragende Icons**. Grund ist gemessen, nicht geschätzt: `--ampel-gelb` erreichte als Text auf der 12-%-Tönung nur **2,66 : 1** und als Grund unter weißer Schrift **3,00 : 1**, `--ampel-gruen` 2,98 bzw. 3,40 – WCAG AA verlangt 4,5 : 1. Die Regel ist mechanisch und per Test abgesichert: **`color:` → Text-Token, `background`/`border` → Vollton**. `--ampel-rot-text` ist wertgleich zu `--ampel-rot` (4,54 / 5,44 – bestand schon) und existiert nur, damit alle drei Textnutzungen durch dieselbe Tür gehen.

> ⚠️ **`[hidden]` ist global geregelt** (`[hidden] { display: none !important; }`, ganz oben in `styles.css`). Die UA-Regel hat Specificity 0-0-1 und verliert gegen jede Klasse mit eigenem `display` – daran ist es dreimal gescheitert: `.quality-panel` (v28), `.pseudo-mini-btn` (v29), `.sample-card` (v58). **Nicht zu verwechseln mit „kein globales `.hidden`"**: diese Konvention betrifft die *Klasse*, mit der Komponenten ihr eigenes Ein-/Ausblenden regeln, nicht das Standard-Attribut. Ein Test legt für **jede** Klasse mit eigenem `display` eine Probe an und prüft, dass `hidden` greift.
...
| Lizenz-Kompatibilität | `LICENSE_TRAITS`, `licenseCompat(a, b)`, `renderLicenseCompat()`, `licCompat` (State) | `#lic-compat-a`, `#lic-compat-b`, `#lic-compat-out`, `.lic-compat-verdict` |
...
| Re-Identifikationsrisiko (k-Anonymität) | `equivalenceClasses()`, `kAnonymitaet()`, `lDiversitaet()`, `riskKey()`, `riskAmpel()`, `riskReport()`, `buildRiskCSV()`, `renderRisk()`, `runRisk()`, `riskFromCleaned()`, `riskCsv` (State), `K_SCHWELLE`, `RISK_SEP` | `#pseudo-tab-risiko`, `#pseudo-risiko-panel`, `#risk-input`, `#risk-cols`, `#risk-run`, `#risk-out`, `[data-risk-qi]`, `#risk-sens`, `#pseudo-csv-risk` |
...
| Rechtspflicht der Organisation | `renderKompassProfil()`, `kompassProfil` (State), `RECHTSPFLICHT_OPTIONS`, Wirkung in `kompassDerived('strategie.recht')` + `hvdVorschlaege()` | `#kompass-profil`, `#kompass-rechtspflicht`, `datenlotse_kompass_profil` |
...
| v59 | **Was passiert, wenn zwei Bestände zusammenkommen.** Das Register führte 20 Lizenzen, sagte aber nichts über ihr Zusammenspiel – wer eigene Daten mit einer fremden Quelle verband, erfuhr das Problem erst hinterher. `licenseCompat(a, b)` benennt jetzt die Pflichten, die sich aus den Lizenztexten ergeben, und wo sie einander widersprechen. Die aufschlussreichste Entwurfsentscheidung steckt in `LICENSE_TRAITS`: `sa` trägt die **Copyleft-Familie**, nicht nur ein `true`. CC BY-SA und ODbL sind beide Share-Alike und gerade deshalb **gegenseitig unvereinbar** – jede verlangt das Ergebnis unter sich selbst. Ein bloßes Flag hätte genau diesen Fall verfehlt und die beiden für kompatibel erklärt. Zweiter nicht offensichtlicher Fall: Copyleft ohne NC trifft auf NC. Das Ergebnis müsste kommerzielle Nutzung zugleich erlauben (Copyleft) und verbieten (NC) – nicht erfüllbar, obwohl beide Lizenzen für sich harmlos aussehen. Und „keine Bearbeitung" schließt jede Zusammenführung aus, auch mit gemeinfreien Daten: eine Zusammenführung *ist* eine Bearbeitung. Bewusst ein **Hinweis, kein Urteil** – der Ergebniskasten sagt ausdrücklich, dass die Lizenz im Original maßgeblich ist und dies keine Rechtsberatung. Nebenbei meldet die Qualitätsprüfung jetzt **verschiedene Lizenzen an den Verteilungen desselben Datensatzes**: DCAT-AP.de erlaubt das, für Nachnutzende ist es eine Falle. 12 neue Tests, die gegen die Lizenztexte rechnen statt gegen die Implementierung; vier Mutationen einzeln geprüft. |
| v58 | **Ein vierter Beispieldatensatz und ein Sprachdurchgang.** Die lauteste Stelle aus der NGO-Auswertung: alle drei Beispielorganisationen waren Behörden. Wer als Verein „Beispiel laden" klickte, sah Bürgeramt, Kämmerei und Stadtplanungsamt – und wusste in dem Moment, dass das Werkzeug nicht für ihn gebaut wurde, egal wie neutral der Hero formuliert war. `data/sample-verein.csv` deckt zwölf Datensätze eines gemeinnützigen Trägers ab und bringt Datenarten mit, für die es bisher kein Beispiel gab: Spenden, Verwendungsnachweise gegenüber Fördermittelgebern, Ehrenamtsstunden, Wirkungskennzahlen – dazu ein „Nicht öffentlich"-Fall, der die v28-Regression an echten Daten absichert. Der Sprachdurchgang trifft die Stellen, an denen „Verwaltung" nur Gewohnheit war: die Pseudonymisierung sprach viermal von **Verwaltungstexten**, obwohl von elf Mustern genau eines verwaltungsspezifisch ist (das Aktenzeichen) – IBAN, E-Mail, Name, Adresse und Geburtsdatum stehen in jedem Beratungsprotokoll. Die RACI-Rollen heißen jetzt „Fachbereich / Ressort" und „IT-Betrieb / Dienstleister", die Musterrichtlinie nennt Vorstand und Geschäftsführung neben der Amtsleitung, verweist beim Zielportal auf beide Wege und setzt keine bestellte DSB mehr voraus (§ 38 BDSG). `guessTheme()` kennt Vorstand, Satzung, Mitglied, Versammlung und Protokoll. Der Demo-Text der Textbereinigung hat einen zweiten Zuschnitt bekommen. **Der Mutationstest hat sich diesmal doppelt gelohnt:** zwei meiner acht neuen Tests waren vakuum – die Themenerkennung prüfte ich mit „Beschlussprotokolle" und „Mitgliederverwaltung", aber „beschluss" und „verwaltung" standen längst im Muster; und `toHaveCount(4)` zählt auch verborgene Karten. Beim Schärfen des zweiten fiel ein echter Defekt im Bestand auf: `.sample-card { display: flex }` überschreibt die UA-Regel `[hidden]`, eine ausgeblendete Karte bliebe also sichtbar – dieselbe Klasse wie `.pseudo-mini-btn[hidden]` in v29 und `.quality-panel.hidden` in v28. Behoben. |
| v57 | **Der Prüfer, der sagt, ob die Bereinigung getragen hat.** Seit v33 lassen sich Spalten einer CSV bereinigen – das Werkzeug sagte aber **nie**, ob das Ergebnis noch auf einzelne Personen zurückführt. Vier Spalten pseudonymisieren und trotzdem eine Zeile behalten, die als einzige „1948 · 04315 · weiblich" trägt, ist keine Anonymisierung. Der dritte Tab der Textbereinigung misst genau das: **k-Anonymität** (wie viele Zeilen stehen in der kleinsten Gruppe?) und optional **l-Diversität** (wie vielfältig ist ein sensibles Merkmal innerhalb dieser Gruppen?). Er passt zur harten Regel aus Modul 3b, weil er **rechnet statt deutet**: Gruppen bilden und zählen, kein ML, kein NER. Und er **misst nur** – Generalisieren oder Unterdrücken wäre eine Entscheidung über fremde Daten; welche Spalten Quasi-Identifikatoren sind, wählt der Mensch. Zwei Entwurfsentscheidungen zeigen in dieselbe Richtung: der Trenner ist `\u0000` statt eines druckbaren Zeichens, sonst fielen `["a b", "c"]` und `["a", "b c"]` auf denselben Schlüssel und zwei Gruppen würden zu einer verschmelzen – k fiele **größer** aus und die Daten sähen **sicherer** aus, als sie sind; und Groß-/Kleinschreibung wird bewusst **nicht** zusammengefasst, aus demselben Grund. `K_SCHWELLE = 5` ist im UI ausdrücklich als Konvention der statistischen Geheimhaltung benannt, nicht als Rechtsvorschrift. Der Weg, um den es geht, ist verdrahtet: aus dem Ergebnis der Spaltenbereinigung führt ein Knopf direkt in die Prüfung. 27 neue Tests, die **von Hand nachrechnen** statt die Implementierung gegen sich selbst zu prüfen; vier Mutationen (naiver Trenner, Case-Folding, entfallene l-Regel, sensibles Merkmal als QI zugelassen) machten je einen Test rot. Nebenbei eine Du-Form aus v51 gefunden, die der Anrede-Test nicht fangen konnte: er prüft Pronomen, „Wähle deshalb je Spalte" ist ein Imperativ. Ein adversarialer Review des eigenen Diffs fand **vier** echte Fehler, den schwersten in der gefährlichen Richtung: der Wechsel des sensiblen Merkmals verwarf das alte Ergebnis nicht, sodass nach dem Umstellen auf die wirklich sensible Spalte weiter „k ≥ 5 erreicht" in Grün auf dem Schirm stand – eine Freigabe für eine Auswahl, die nie berechnet wurde. Dazu: der Fokus sprang beim Umschalten eines Merkmals per Tastatur auf `<body>` (dieselbe Lehre wie bei der Massenbearbeitung, nur nicht angewandt), der Knopf war bei einer Tabelle aus reiner Kopfzeile aktiv und tat wortlos nichts, und die Zusammenfassung stand als weiße Schrift auf `--ampel-gelb` bei **3,0 : 1**. Alle vier behoben, jeder mit einem Test belegt; der Kontrast wird gemessen. Beim Nachmessen fiel auf, dass die **bestehenden** Badges dieselbe Schwäche haben – notiert, aber bewusst nicht in diesem Schnitt behoben. |
| v56 | **Drei Stellen, an denen das Werkzeug für richtiges Verhalten bestrafte.** Eine Bestandsaufnahme nach v55 – wo setzt DatenLotse stillschweigend eine Behörde voraus? – förderte weniger zutage als befürchtet: Hero, FAQ, Rundgang und vier der fünf Musterdokumente sprechen längst „Ihre Organisation" an. Die Reibung saß konzentriert, und drei Punkte davon waren keine Zielgruppenfrage, sondern derselbe Fehler wie bei `strategie.recht` vor v55. **(1)** Der Governance-Fragebogen kannte kein „nicht relevant", der Kompass seit jeher. Die DSB-Frage wiegt 12 Punkte, nach § 38 BDSG muss aber erst ab 20 Personen jemand bestellt werden – ein kleiner Verein konnte sie nie erreichen und bekam eine Lücke ausgewiesen, die keine war. `reifegrad()` normiert jetzt über die **bewertbaren** Gewichte; ohne ein einziges `na` ist das Ergebnis rechnerisch identisch zu vorher, es braucht also keine Migration. Sind alle acht abgewählt, gibt es keinen Nenner – dann steht „Keine bewertbare Frage" statt „Lückenhaft", denn bewertet wurde nichts. **(2)** „Amtliches Werk – lizenzfrei nach § 5 UrhG" stand jeder Organisation im Dropdown, obwohl die Lizenzfreiheit an der amtlichen **Herkunft** hängt und nicht an einer Entscheidung des Herausgebers; `licenseIsOpen()` konnte es nicht fangen, weil `open: true` korrekt ist. Der Eintrag **bleibt** im Register – fremde Kataloge bringen ihn mit, Ausblenden würde ihn beim Rückimport still verschlucken –, aber die Qualitätsprüfung meldet ihn als Fehler, sobald die Rechtspflicht mit „nein" beantwortet ist. **(3)** Der Prüfpunkt „Zielportal" nannte GovData, kommunales Portal und CKAN – zwei der drei stehen nur öffentlichen Stellen offen, der Punkt führte also ins Leere statt zu helfen. Dazu eine Sprachkorrektur: der Lizenz-Wegweiser ließ zwischen „Deutschland / **Verwaltung**" und „International" wählen, obwohl die Frage den Geltungsraum meint und nicht den Sektor. 16 neue Tests. Ein adversarialer Review des eigenen Diffs fand zwei echte Fehler **in der Korrektur selbst**: der neue Badge-Zweig setzte keinen Ampel-Modifier und damit keinen Hintergrund – weiße Schrift auf hellem Grund, Kontrast 1,03 : 1 – und von fünf Konsumenten des geänderten `reifegrad()`-Vertrags waren nur drei nachgezogen: Status-Einseiter und RACI-CSV meldeten weiter „0/100 · Lückenhaft“, also genau die Aussage, die der Rest des Diffs gerade als falsch verwarf. Beide behoben und mit Tests belegt – der Kontrast wird dabei **gemessen**, nicht am Text geprüft, weil ein `toContainText` auf unsichtbarem Text grün bleibt. Zwei Mutationstests machten 4 bzw. 3 Tests rot. |
| v55 | **Eine Frage nach der Rechtspflicht – statt einer nach dem Sektor.** Das Werkzeug behandelte jede Organisation gleich und setzte dabei stillschweigend eine öffentliche Stelle voraus: der Prüfpunkt „Rechtsgrundlagen sind geklärt" blieb bei einem Verein dauerhaft offen und zog dessen Reifegrad herunter, obwohl er nichts falsch macht, und die Qualitätsprüfung schlug hochwertige Datensätze nach der DVO (EU) 2023/138 vor, die ihn gar nicht bindet. Naheliegend wäre ein Schalter „Behörde oder NGO" gewesen – **bewusst nicht gebaut**: dazwischen liegen Stadtwerke, Verkehrsbetriebe, Hochschulen, Sparkassen und Belehnte, die ein Zwei-Wege-Schalter falsch einsortiert, und die Sektor-Bezeichnung selbst ändert am Werkzeug nichts. Gefragt wird deshalb direkt nach dem, was tatsächlich etwas ändert: der **Pflicht**. Die Angabe ist optional (DatenLotse startet ohne Einrichtung; eine Pflichtfrage davor erzwänge eine Entscheidung vor dem Verstehen) und steht unter dem Score, nicht davor. Sie wirkt an **genau zwei** Stellen und blendet nichts aus. Beim Nachzählen der 33 Prüfpunkte fiel auf, dass nur **einer** strikt an der Pflicht hängt – eine frühere Einschätzung von vier war falsch und ist korrigiert. |
| v54 | **Achte Kompass-Dimension: Können & Kapazität.** Ein Abgleich mit dem *Data Maturity Framework* von Data Orchard (v2.1) zeigte eine Lücke, die der Kompass strukturell hatte: er maß, ob eine Organisation etwas **getan** hat – nicht, ob sie es **weiter** kann. Der Governance-Fragebogen fragt nach Rollen (Owner, Steward, DSB), aber an keiner Stelle nach Können. Die neue Dimension fragt nach Fachwissen, Schulung, gesicherter Fortführung und externer Unterstützung. Dazu zwei geschärfte Prüfpunkte: die **Ressourcenfrage** bei *Strategie* (Data Orchards „Investment") und die **Selbstprüfung** bei *Wirkung* (deren „Self-questioning") – letztere leitet sich aus den festgehaltenen Kompass-Ständen ab, weil wer Stände festhält, seinen Stand hinterfragt. Damit 8 Dimensionen und 33 Prüfpunkte. **Nicht übernommen** wurden die Themen *Analysis* und *Tools*: Inhaltsanalyse ist ein anderes Produkt, Systemarchitektur ist bereits als Phase 4 & 5 der Beratung zugeordnet. ⚠️ Das Rahmenwerk steht unter **CC BY-NC-SA 4.0** – nach DatenLotses eigener `licenseIsOpen()` also **nicht offen**, und SA wäre mit der GPL-3.0 unvereinbar. Übernommen wurde deshalb nur die **Anregung**: alle Prüfpunkte sind eigene Formulierungen, das Modell steht als Quelle in `METHOD_MODELS` und in „Methodik & Quellen". |
| v53 | **Zählskript wie beim Hersteller, Browser-Cache in der CI.** Das Zähl-Skript stand auf `defer`; jetzt `async`, wie im GoatCounter-Snippet. Beides zählt gleich, aber es gibt keinen Grund, ohne Not vom Hersteller-Snippet abzuweichen. Dazu ein Cache für die Playwright-Browser: der Download war der langsamste Schritt jedes Laufs und die einzige Stelle, an der einer haengen blieb – bei v52 zehn Minuten statt einer, bis der Lauf abgebrochen werden musste. Der `install`-Aufruf bleibt trotzdem stehen: er setzt die apt-Abhängigkeiten und läuft bei vorhandenem Browser in Sekunden durch. Cache-Schlüssel ist `package.json`, weil `@playwright/test` dort exakt gepinnt ist und das Lockfile bewusst nicht im Repo liegt. |
